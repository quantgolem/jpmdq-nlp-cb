...
from jpmdq_nlp_cb_utils import (
    ...
    get_last_ingested_date, get_spark, upsert_manifest, write_day_to_volume,
    yesterday_ymd,
)
...
DEFAULT_START  = None          # None → 2 years ago; used when group has no manifest data
...
default_start = DEFAULT_START or two_years_ago
...
for group_id in GROUPS:
    last = None if DRY_RUN else get_last_ingested_date(spark, manifest_table, group_id)
    if last:
...
print(
    ...
    f"  bronze={bronze_table}  dry_run={DRY_RUN}",
    flush=True,
)
